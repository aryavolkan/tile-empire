...
var expansion_costs: Dictionary = {}  # maps player_id to expansion cost int
var border_tensions: Dictionary = {}  # maps "player1_player2" key to float tension
...
func get_expandable_tiles(player_id: int) -> Array[Tile]:
    ...
    if _territory_frontier and tile_map:
        ...
        var frontier_positions: Array = \
            _territory_frontier.get_frontier(owner_grid, player_id, w, h)
...
func register_settlement(_settlement: Node, player_id: int) -> void:
...
func get_adjacent_unowned_tiles(_world_position: Vector2, player_id: int) -> Array:
